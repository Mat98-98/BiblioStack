        ...
        <div className="rounded-xl border border-border bg-card p-4 space-y-3">
            ...
            <div className="flex flex-col gap-2">
                ...
                <CreateNewButton
                    ...
                    selected={resolution !== undefined && resolution?.authorId === null}
...
export default function AuthorConflictStep({ conflicts, onResolve, onBack, loading }) {
    const [resolutions, setResolutions] = useState(() => {
        const initial = new Map()
        conflicts.forEach(c => {
            if (c.suggestedMatch !== null) {
                initial.set(c.inputName, { inputName: c.inputName, authorId: c.suggestedMatch })
            }
        })
        return initial
    })
    ...
    const handleResolve = (inputName, authorId) => {
        setResolutions(prev => new Map(prev).set(inputName, { inputName, authorId }))
    }

    const allResolved = conflicts.every(c => resolutions.has(c.inputName))
    ...
    return (
        <div className="space-y-6">
            <div className="space-y-1">
                <h2 className="text-lg font-semibold">Risolvi conflitti autori</h2>
                <p className="text-sm text-muted-foreground">
                    Per {conflicts.length} {conflicts.length === 1 ? "autore" : "autori"} sono stati
                    trovati candidati simili nel database. Seleziona quale usare o crea un nuovo autore.
                </p>
            </div>
            ...
            <div className="flex gap-3">
                ...
                <Button className="flex-1" onClick={handleSubmit} disabled={!allResolved || loading}>
                    {loading
