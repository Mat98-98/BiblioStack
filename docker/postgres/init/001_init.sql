...
CREATE TABLE IF NOT EXISTS suspensions (
                                           id int GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
                                           user_id bigint NOT NULL REFERENCES users(id) ON DELETE NO ACTION, -- FK
    handled_by bigint NOT NULL REFERENCES users(id) ON DELETE NO ACTION, --FK bibliotecario, il sistema avrà un utente in modo da monitorare le sospensioni automatiche
    reason text,
...
CREATE TABLE IF NOT EXISTS loans (
                                     id bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
                                     user_id bigint NOT NULL REFERENCES users(id) ON DELETE NO ACTION, --FK
    handled_by bigint NOT NULL REFERENCES users(id) ON DELETE NO ACTION, -- FK bibliotecario
...
CREATE TABLE IF NOT EXISTS notices (
                                       id int GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
                                       user_id bigint NOT NULL REFERENCES users(id) ON DELETE NO ACTION,
    handled_by bigint NOT NULL REFERENCES users(id) ON DELETE NO ACTION, -- Il sistema avrà un utente in modo da monitorare le segnalazioni automatiche
    notice_type_id smallint NOT NULL REFERENCES notice_types(id) ON DELETE NO ACTION ON UPDATE CASCADE,
    loan_id bigint NOT NULL REFERENCES loans(id) ON DELETE NO ACTION,
    description text,
